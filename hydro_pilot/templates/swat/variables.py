...
def calcSwatOutputRows(
    ...
    Args:
        ...
        subbasin: Subbasin ID (1-based). Required for all types.
        period: Time period filter. See _parsePeriod for formats.
        timestep: Override meta timestep ("daily"/"monthly"/"yearly").
        hru: Local HRU index (1-based). Required when outputType is "hru".
    ...
    if outputType in ("rch", "sub"):
        ...
        if unitIndex < 1 or unitIndex > nUnits:
            raise ValueError(
                f"subbasin {subbasin} out of range [1, {nUnits}]"
            )
    elif outputType == "hru":
        if hru is None:
            raise ValueError("hru is required when outputType is 'hru'")
        nUnits = _calcNHrusTotal(meta)
        unitIndex = _resolveHruIndex(meta, subbasin, hru)
    else:
