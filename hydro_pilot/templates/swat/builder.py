...
def _autoPhysical(designItem: Dict[str, Any], paramDb: Dict[str, Any]) -> Dict[str, Any]:
    """Auto-generate a physical param entry from a design item + param_db."""
    name = designItem["name"]
    dbEntry = paramDb.get(name)
    if not dbEntry:
        raise ValueError(
            f"Unknown parameter '{name}': not found in param_db and no "
            f"inline location provided. Either add it to param_db.yaml or "
...
def _mergePhysical(
    ...
    """Merge a physical entry with param_db defaults, optionally paired with design."""
    name = physItem["name"]
    dbEntry = paramDb.get(name)

    # Location priority: inline > param_db > error
    ...
    if not location:
        if dbEntry:
            ...
            raise ValueError(
                f"Parameter '{name}' has no inline location and is not in "
                f"param_db. Provide a location in the physical entry."
            )

    # Type priority: physical explicit > param_db > default "float"
    paramType = physItem.get("type")
    if not paramType:
        paramType = dbEntry["type"] if dbEntry else "float"

    # Bounds: design explicit > param_db > None
...
def resolvePhysicalParams(
    ...
    Three scenarios:
    A) design only → auto-generate physical from param_db
...
def expandLocations(
    ...
    - Without filter: keep original glob/regex pattern for ParamManager to resolve.
    ...
    for pp in physicalParams:
        ...
        if hasFilter and not _isGlobalFile(filePattern):
            ...
            # No filter or global file: keep pattern for ParamManager
...
def buildSwatParams(
    ...
    Args:
        ...
        paramDb: SWAT_PARAM_LIBRARY dict.
    ...
    # Step 1: Resolve physical params (fill defaults from param_db)
