...
- 像 `version: swat` 这样的版本会通过 `models.registry` 中的模板注册表解析
...
- 底层的配置准备和校验逻辑是复用的
- CLI 专属行为不会被复用
- 直接运行时遇到配置错误会抛异常，不像 CLI 那样打印 diagnostics 再返回退出码
...
新的模型族可以通过 `ModelTemplate` 接口和模板注册表接入。
